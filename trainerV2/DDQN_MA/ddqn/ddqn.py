...
class DDQN(object):
    def __init__(self, env, config, network_config, eval_name = 'ddqn_eval', target_name = 'ddqn_target') -> None:
        ...
        self.eval_net = MLP(inputs=self.inputs, outputs=self.outputs, name=self.eval_name, fc_dim1=network_config['MLP']['FC1'], fc_dim2=network_config['MLP']['FC2'], chkpt_dir=self.output_dir).to(device=device)
        self.target_net = MLP(inputs=self.inputs, outputs=self.outputs, name=self.target_name, fc_dim1=network_config['MLP']['FC1'], fc_dim2=network_config['MLP']['FC2'], chkpt_dir=self.output_dir).to(device=device)
