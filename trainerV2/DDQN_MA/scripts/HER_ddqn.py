...
import numpy as np
from trainerV2.DDQN_MA.scripts.networks import MLP
from trainerV2.DDQN_MA.scripts.HERBuffer import HindsightExperienceReplayMemory as ReplayBuffer
...
class DDQN(object):
    def __init__(self, env, config, network_config, eval_name = 'her_ddqn_eval', target_name = 'her_ddqn_target') -> None:
        ...
        self.eval_net = MLP(inputs=self.inputs, goals=len(self.goal), outputs=self.outputs, name=self.eval_name, \
                                fc_dim1=network_config['MLP']['FC1'], fc_dim2=network_config['MLP']['FC2'], chkpt_dir=self.output_dir).to(device=device)
