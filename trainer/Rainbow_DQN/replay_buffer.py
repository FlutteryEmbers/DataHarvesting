...
class Prioritized_ReplayBuffer(object):
    ...
    def sample(self, total_steps):
        batch_index, IS_weight = self.get_batch_index()
    ...
    def get_batch_index(self, ):
        batch_index = np.zeros(self.batch_size, dtype=np.long)
        IS_weight = torch.zeros(self.batch_size, dtype=torch.float32)
        priority_sum = self.sum_tree.priority_sum  # 树的顶端保存了所有priority之和
        segment = priority_sum / self.batch_size  # 把[0,priority_sum]等分成batch_size个区间，在每个区间均匀采样一个数
        for i in range(self.batch_size):
            ...
            batch_index[i] = index
            prob = priority / priority_sum  # 当前数据被采样的概率
    ...
    def update_priority(self, index, td_error):
        priority = (np.abs(td_error) + 0.01) ** self.alpha
...
class N_Steps_Prioritized_ReplayBuffer(object):
    ...
    def get_batch_index(self, ):
        batch_index = np.zeros(self.batch_size, dtype=np.long)
        IS_weight = torch.zeros(self.batch_size, dtype=torch.float32)
        priority_sum = self.sum_tree.priority_sum
        segment = priority_sum / self.batch_size  # 把[0,priority_sum]等分成batch_size个区间，在每个区间均匀采样一个数
        for i in range(self.batch_size):
            a = segment * i
            b = segment * (i + 1)
            v = np.random.uniform(a, b)
            index, priority = self.sum_tree.get_index(v)  # 采样一个数据的index，已经其当前的priority
            batch_index[i] = index
            prob = priority / priority_sum  # 当前数据被采样的概率
            IS_weight[i] = (self.current_size * prob) ** (-self.beta)  # 重要性采样权重
        IS_weight /= IS_weight.max()  # normalization

        return batch_index, IS_weight

    def update_priority(self, index, td_error):  # 根据传入的td_error，更新index所对应数据的priority
        priority = (np.abs(td_error) + 0.01) ** self.alpha
        self.sum_tree.update(data_index=index, priority=priority)
